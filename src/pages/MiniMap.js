                  ...
                  ) : (<GoogleMap mapContainerStyle={mapStyles} zoom={defaultCenter.zoom} center={defaultCenter.center} >
        {
            myCameras.map(tempCam => {
              ...
              if(location.lat==defaultCenter.center.lat &&location.lng==defaultCenter.center.lng ){
                return (<Marker icon="https://maps.google.com/mapfiles/ms/icons/green-dot.png" key={tempCam.id} position={getLocation(tempCam)} onClick={()  => handleClick(tempCam)} />)
              }else{
                return(<Marker icon="https://maps.google.com/mapfiles/ms/icons/red-dot.png" key={tempCam.id} position={getLocation(tempCam)} onClick={()  => handleClick(tempCam)} />
              )}
